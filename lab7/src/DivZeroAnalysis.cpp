...
namespace dataflow
{
  ...
  bool DivZeroAnalysis::check(Instruction *Inst)
  {
     ...
     */
    // Check if the instruction is a signed or unsigned division instruction
    if (isa<BinaryOperator>(Inst))
    {
      ...
      if (BinOp->getOpcode() == Instruction::SDiv || BinOp->getOpcode() == Instruction::UDiv)
      {
        ...
        // Get the abstract value of the divisor from the OutMap
        Memory *OutMem = OutMap[Inst];
        Domain *DivisorDomain = getOrExtract(OutMem, Divisor);

        // Check if the divisor is either Zero or MaybeZero
